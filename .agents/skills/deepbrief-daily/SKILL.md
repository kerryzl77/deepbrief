...
- For repository/source-reconstruction course books, default to source recovery, not advisory redesign. A request that the reader should be able to "rebuild" the system means recover the inspected source clearly enough to reproduce its architecture: user journeys, control flow, data flow, files, states, variables, permissions, and runtime boundaries. Do not add opinionated "best-in-class" improvements, MVP roadmaps, module build metrics, test strategies, or rebuild checklists unless the current user explicitly asks for advisory design content after any recent `feedback.md` source-only preference has been surfaced or resolved. If advisory material is requested, label it as advisory and keep it separate from source facts.
